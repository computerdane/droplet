...
func test_fetch_does_not_move_a_view_the_user_moved() -> void:
	...
	_arrive(main, dir, KTST[0], KTST[0])
	check_eq(main.frame, 1, "starting playback also suspends the old request")
	main.fetcher._finish(job, 0)
	_close(main)
	_remove_dir(dir)


func test_stale_live_job_first_volume_does_not_switch_to_live() -> void:
	...
	check(not main.window.live and not main.live, "late report keeps the fixed window")
	check_eq(main.volume.name, KTST[0], "late report keeps the selected scan")
	TimeWindow.clock_override = -1
	_close(main)


func test_explicit_window_precedence_and_uncached_permalink() -> void:
	TimeWindow.clock_override = T0 + 360
	for opts in [
		["event=moore2013", "window=live"], ["site=KTST", "window=live", "time=20240501_220000"]
	]:
		var main := _launch(opts)
		check(main.window.live and main.live, "explicit live window wins over time/event")
		_close(main)
	var main := _launch(["site=KTLX", "time=20240501_220000"])
	check_eq(main.site, "KTLX", "uncached site remains selected for its arriving scans")
	check(main.volume == null and not main.window.live, "historical request awaits its scan")
	_close(main)
	TimeWindow.clock_override = -1
...
func test_changing_the_window_stops_jobs_outside_it() -> void:
	...
	main._step(-1)
	check(main.window.live and not again.stopped, "a step keeps the rolling window and fetch")
	...
	check(event.stopped and inside.running, "narrowing stops the old request beyond its bounds")
...
func test_panel_modes_set_the_window() -> void:
	...
	check_eq(main.fetcher.running_jobs().size(), 4, "updates remain in range; live also follows")
	main._set_live(false)
...
## Navigation suspends following without changing mode; arriving/backfilled scans cannot
## undo it, the range rolls, and a selection aging out advances to the oldest survivor.
func test_live_navigation_keeps_window_and_fetches() -> void:
	for move in ["left", "right", "slider", "vwp"]:
		TimeWindow.clock_override = T0 + 360
		var dir := _temp_dir()
		var main := _launch(["volumes=" + dir, "site=KTST", "window=live:10"])
		main._pick_site("KTST")
		...
		if move == "left":
			...
			main.hud.scrubbed.emit(0)
		else:
			main.hud.vwp.frame_picked.emit(KTST[0])
		var selected: String = main.volume.name
		check(main.window.live and not main.live, move + ": rolling, follow suspended")
		check(not main.live_timer.is_stopped() and not job.stopped, move + ": timer/fetch stay")
		_arrive(main, dir, KTST[1], "KTST_20240501_220600")
		job.volumes.append(KTST[1])
		main.fetcher.job_updated.emit(job)  # first report must not resume following
		check_eq(main.volume.name, selected, move + ": arrivals do not jump")
		TimeWindow.clock_override = T0 + 901
		main._on_live_tick()
		check_eq(main.volume.name, "KTST_20240501_220600", move + ": oldest survivor")
		check(not main.live and main.window.live, move + ": still browsing live")
		_arrive(main, dir, KTST[1], "KTST_20240501_221400")
		check_eq(main.volume.name, "KTST_20240501_220600", move + ": remains selected")
		main._toggle_live()
		check(main.live and main.window.live, move + ": explicit Live resumes")
		check_eq(main.volume.name, "KTST_20240501_221400", move + ": newest")
		_close(main)
		_remove_dir(dir)
	TimeWindow.clock_override = -1


func test_finished_fetch_cannot_take_over_after_navigation() -> void:
	var main := _launch(["site=KTST", "time=20240501_220000", "live=0"])
	var job: Fetcher.Job = main.fetcher.start_update("KTST", "2024-05-01T22:00Z")
	job.volumes.append(KTST[0])
	main._step(1)
	main.fetcher._finish(job, 0)
	check_eq(main.volume.name, KTST[1], "completion cannot undo a step")
	job = main.fetcher.start_update("KTST", "2024-05-01T22:00Z")
	job.volumes.append(KTST[0])
	main._pick_site("KTSU")
	main.fetcher._finish(job, 0)
	check_eq(main.site, "KTSU", "completion cannot switch back to its site")
	job = main.fetcher.start_update("KTSU", "2024-05-01T22:02Z")
	job.volumes.append("KTSU_20240501_220200")
	main._set_window(TimeWindow.around(T0 + 600))  # overlaps the job, so it can finish
	var expected: TimeWindow = main.window
	main.fetcher._finish(job, 0)
	check(main.window == expected, "completion cannot restore an earlier overlapping window")
	_close(main)
...
## A panel request retargets the live view; returning to the overview stops automatic
## followers (including archive fallback) while preserving explicit update requests.
func _check_live_fetch_ownership() -> void:
	TimeWindow.clock_override = T0 + 360
	for can_live in [true, false]:
		var main := _launch(["site=KTLX", "window=live:20", "mosaic=1"])
		main.fetcher.can_live = can_live
		main._pick_site("KTLX")
		var old_jobs: Array = main.fetcher.running_jobs().duplicate()
		main.hud.fetch_panel.update_requested.emit("KAMX", "", "", "")
		check_eq(main.site, "KAMX", "Newest selects its requested site")
		check(main.window.live and main.live, "Newest keeps the live window and following")
		for job: Fetcher.Job in old_jobs:
			check(job.stopped, "previous site's distant live/mosaic request stops")
		var explicit: Fetcher.Job = main.fetcher.jobs[-1]
		check_eq(explicit.key, "update KAMX newest", "explicit newest request remains")
		var followers: Array = main.fetcher.running_jobs().filter(
			func(j: Fetcher.Job) -> bool: return j != explicit
		)
		check_eq(followers.size(), 5, "selected site and mosaic follow the live window")
		var before: TimeWindow = main.window
		main._show_overview()
		check(main.overview and main.window == before, "overview keeps the shared window")
		for job: Fetcher.Job in followers:
			check(job.stopped, "overview stops each automatic live/fallback request")
		check(explicit.running and not explicit.stopped, "explicit request can finish")
		check_eq(main.fetcher.running_jobs().size(), 1, "only explicit download remains")
		main.hud.fetch_panel.update_requested.emit("KAMX", "", "", "")
		check(not main.overview and main.live, "Newest from overview resumes live following")
		_close(main)
	TimeWindow.clock_override = -1
